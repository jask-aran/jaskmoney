...
- Net/Cashflow adds `spend_vs_budget_pace`: cumulative debits vs cumulative month-prorated budget pace across the active timeframe.
...
- `dispatch_test.go` — contract coverage, intent mapping, kind defaults, footer
  rendering tests
- `flow_test.go` — intent-behavior and confirm/save persistence flow tests
...
- [x] Every overlay and tab scope has a registered `InteractionContract`
- [x] Footer hints in all contexts are generated from contract data (no more
      manual hint strings in render functions)
- [x] Contract intents map to valid keys in `KeyRegistry` for every scope
- [x] Test matrix validates handler behavior against declared intents for
      all reachable states
- [x] `IntentConfirm` vs `IntentSave` distinction is explicit in all form
      contexts that persist data
- [x] All `modalTextContracts` entries have corresponding
      `InteractionContract` entries with compatible intent declarations
- [x] No regression in existing UI behavior (all Phase 1-6 acceptance
      criteria still pass)
- [x] Footer rendering migration is complete for all contexts (no mixed
      manual/contract-driven footers)

#### Acceptance Notes (v0.39.1)

- Contract-driven footer resolution is now centralized through
  `activeInteractionContract()` + `renderFooterFromContract()`.
- Interaction contracts are registered for overlay scopes, tab scopes, and
  modal text scopes; tests enforce coverage and key mapping integrity.
- Flow regressions verify intent behavior (`cancel` in command palette) and
  confirm/save persistence paths (manager modal save, settings tag form save).
- Full suite validated with `go test ./...`.
