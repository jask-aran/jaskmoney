...
### Phase 1 Completion Notes (v0.21)

Status: **Complete**

Implemented additions:
- Added centralized `KeyRegistry` with scoped action lookup and footer help generation.
- Replaced ad-hoc key map usage with registry-driven bindings in model/update flow.
- Added config-driven key override support with normalization for standard combinations (`ctrl+...`, `shift+...`, etc.).
- Added tests for scope lookup, duplicate prevention, help ordering, conflict detection, and key normalization.

Acceptance check:
- [x] All existing keybindings work identically
- [x] Footer help text is generated from registry
- [x] No hard-coded binding lists remain in footer construction paths
...
- `Esc` — clears search first (if active), then highlighted range, then selected rows
...
**Acceptance:**
- Space toggles, Shift+Up/Down highlights ranges, Space toggles highlighted range, Esc clears in priority order (search -> highlight -> selection)
...
**Goal:** Multi-account support centered in the Manager tab, with import-time account assignment from config and persisted account scoping across Manager/Dashboard.
...
Migration policy for v0.26 rebaseline: **fresh schema only**. Backward migration/data preservation paths are out of scope.
...
2. If not found, import fails with account mapping error
3. `importCSV()` receives `accountID` parameter, sets on INSERT
4. Amounts are imported as parsed from CSV (no account-type sign transformation)

### Account scope UX (Manager)

- Manager is the only account scope surface in v0.26
- Multi-select scope with Space toggle on focused account
- `s` confirms and saves selection to `account_selection` table (persisted)
...
Account CRUD lives in Manager (create/edit/delete, plus clear/nuke flows).
...
**Tests:**
- `db_test.go`: fresh schema behavior, account CRUD, selection persistence
- `ingest_test.go`: account assignment, missing account failure, no sign-flip behavior
- `model_test.go`: manager scope/filter integration (if covered in model tests)

**Acceptance:**
- Accounts CRUD in Manager
- Import auto-detects account from config mapping and sets `transactions.account_id`
- Import amounts are preserved as parsed (no credit/debit sign transform)
- Manager account scope filters both Manager transactions and Dashboard
- Selection persists across restarts

### Phase 6 Completion Notes (v0.26)

Status: **Accepted**

Implemented additions:
- Added accounts schema and account selection persistence (`accounts`, `account_selection`, `transactions.account_id`) in the v0.26 baseline schema.
- Added account-aware transaction loading and filtering, including persisted account scope applied across Manager and Dashboard.
- Added Manager account workflows for create/edit/delete, clear account transactions, and account nuke flow.
- Added config-backed account mapping for imports and import-time account resolution, with explicit failure when mapping is missing.
- Preserved imported transaction amounts as parsed (no account-type sign inversion).
- Added account indicator in header and account column behavior in transaction rendering.

Acceptance check:
- [x] Accounts CRUD in Manager
- [x] Import auto-detects account from config mapping and sets `transactions.account_id`
- [x] Import amounts are preserved as parsed (no credit/debit sign transform)
- [x] Manager account scope filters both Manager transactions and Dashboard
...
### Phase 7 Completion Notes (v0.27)

Status: **Accepted**

Implemented additions:
- Tags are first-class transaction metadata with dedicated transaction table rendering.
- Tag CRUD is available in Settings, including color assignment and editing flows.
- Quick tag picker supports fuzzy selection, multi-select apply, and inline create.
- Tag rules are persisted and applied during import alongside category rules.
- Mandatory `IGNORE` tag is seeded/enforced and excludes tagged transactions from dashboard spending charts.

Acceptance check:
- [x] Tags CRUD in Settings (global + scoped)
- [x] `t` opens fuzzy tag picker with scoped tags first
- [x] Inline create works
- [x] Bulk tagging works
- [x] Tag rules auto-apply on import
- [x] Tags display in table and detail
...
### Phase 8 Completion Notes (v0.28)

Status: **Accepted**

Implemented additions:
- Added a dedicated `commands.go` command system with shared metadata, enable/disable checks, and execution wiring for 11 shipped commands.
- Implemented modal Command Palette (`Ctrl+K`) with fuzzy search, ranked matches, disabled-state visibility, and keyboard execution flow.
- Implemented footer Command Mode (`:`) with inline input and floating suggestions, backed by the same registry and ranking logic.
- Added in-session MRU promotion for the last executed command and first-enabled fallback execution when top results are disabled.
- Added persisted command-interface default setting (`palette`/`colon`) and Settings toggle control in Database & Imports.
- Wired tag-rule application command path (`applyTagRules`) into the Tea message loop with refresh/status handling.

Acceptance check:
- [x] Ctrl+K opens palette with fuzzy search
- [x] `:` opens command mode in footer
- [x] Commands execute correctly
- [x] Both interfaces share one command registry
...
### Phase 9 Completion Notes (v0.3)

Status: **Accepted**

Implemented additions:
- Added global number-key tab navigation (`1` Dashboard, `2` Transactions/Manager, `3` Settings).
- Hardened modal dismiss consistency so `Esc` always closes the current topmost modal (including detail-notes sub-mode).
- Kept footer help registry-driven across all runtime scopes, including tag editor field navigation.

Acceptance check:
- [x] Number keys navigate tabs
- [x] Consistent Esc behavior across all modals
- [x] All footers auto-generated from registry
- [x] Clean, polished v0.3 release
...
Phase 6: Accounts (v0.26) [needs Phases 1-4]
    |
Phase 7: Tags (v0.27) [accepted; needs Phases 4, 5, 6]
    |
