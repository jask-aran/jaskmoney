...
- `core/`: framework and architecture primitives (router, model contracts, keys/commands, generic screens/widgets/db helpers).
- `app/`: Jaskmoney-specific implementation (tab definitions/layouts, app command wiring, app-specific screen composition).
- `main.go`: bootstrap entrypoint (wire config/db/imports, build model, start Bubble Tea).
...
- Keep package boundaries aligned with the v2 split: framework in `core/`, product implementation in `app/`.
...
- Add focused tests near each package (`core/router_test.go`, `core/widgets/layout_test.go`, `app/tabs_test.go`, etc.) to validate contracts and app wiring.
...
- Porting rule of thumb:
  - Put reusable mechanics in `core/`.
  - Put Jaskmoney behavior/UI composition in `app/`.
