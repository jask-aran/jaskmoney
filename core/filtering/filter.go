...
func parseFilterStrict(input string) (*Node, error) {
	return parseFilterWithMode(input, true)
}

func parseFilterWithMode(input string, strict bool) (*Node, error) {
...
func fallbackPlainTextFilter(input string) *Node {
	text := strings.TrimSpace(input)
	if text == "" {
		return nil
	}
	return &Node{kind: filterNodeText, op: "contains_meta", value: text}
}
...
func (p *filterParser) parseExpr() (*Node, error) {
	return p.parseOrExpr()
}

func (p *filterParser) parseOrExpr() (*Node, error) {
	...
	return &Node{kind: filterNodeOr, children: flattenFilterChildren(filterNodeOr, children)}, nil
}

func (p *filterParser) parseAndExpr() (*Node, error) {
	...
	return &Node{kind: filterNodeAnd, children: flattenFilterChildren(filterNodeAnd, children)}, nil
}
...
func (p *filterParser) parseUnary() (*Node, error) {
	if p.peek().kind == filterTokNot {
		...
		return &Node{kind: filterNodeNot, children: []*Node{child}}, nil
	}
	return p.parseTerm()
}

func (p *filterParser) parseTerm() (*Node, error) {
	tok := p.peek()
	switch tok.kind {
	...
	case filterTokQuoted:
		q := p.consume()
		return &Node{kind: filterNodeText, op: "contains", value: q.text}, nil
	case filterTokWord:
		if p.isFieldPredicateAt(p.idx) {
			return p.parseFieldPredicate()
		}
		w := p.consume()
		return &Node{kind: filterNodeText, op: "contains", value: w.text}, nil
	default:
...
func (p *filterParser) parseFieldPredicate() (*Node, error) {
	fieldTok := p.consume()
	...
	case "type":
		...
		return &Node{kind: filterNodeField, field: field, op: "=", value: v}, nil
	case "cat", "tag", "acc":
		raw, err := p.collectFieldValue(field, true)
		if err != nil {
			return nil, err
		}
		return &Node{kind: filterNodeField, field: field, op: "=", value: strings.TrimSpace(raw)}, nil
	case "desc", "note":
		raw, err := p.collectFieldValue(field, true)
		if err != nil {
			return nil, err
		}
		return &Node{kind: filterNodeField, field: field, op: "contains", value: strings.TrimSpace(raw)}, nil
	default:
...
func parseAmountField(raw string, pos int) (*Node, error) {
	...
	if strings.Contains(v, "..") {
		...
		return &Node{kind: filterNodeField, op: "..", valueLo: canonicalFloat(lo), valueHi: canonicalFloat(hi)}, nil
	}
	ops := []string{"<=", ">=", "<", ">", "="}
	for _, op := range ops {
		if strings.HasPrefix(v, op) {
			...
			return &Node{kind: filterNodeField, op: op, value: canonicalFloat(n)}, nil
		}
	}
	...
	return &Node{kind: filterNodeField, op: "=", value: canonicalFloat(n)}, nil
}
...
func parseDateField(raw string, pos int) (*Node, error) {
	v := strings.ReplaceAll(strings.TrimSpace(raw), " ", "")
	if v == "" {
	...
	if strings.Contains(v, "..") {
		...
		return &Node{kind: filterNodeField, op: "..", valueLo: lo, valueHi: hi}, nil
	}
	canon, err := canonicalDateToken(v)
	if err != nil {
		return nil, fmt.Errorf("date: invalid value %q", raw)
	}
	return &Node{kind: filterNodeField, op: "=", value: canon}, nil
}
...
func validateStrictGrouping(node *Node) error {
	if node == nil {
		return nil
	}
...
func hasStrictMixedGroupingViolation(node *Node) bool {
	if node == nil {
		return false
	}
...
func evalFilter(node *Node, t Row) bool {
	if node == nil {
		return true
	}
	switch node.kind {
	case filterNodeText:
		...
		return evalTextFilter(node.op, needle, t)
	case filterNodeField:
		return evalFieldFilter(node, t)
	case filterNodeAnd:
		for _, child := range node.children {
...
func evalTextFilter(op, needle string, t Row) bool {
	...
	if op == "contains_meta" {
		...
		for _, tg := range t.TagNames {
			if strings.Contains(strings.ToLower(strings.TrimSpace(tg)), needle) {
				return true
			}
		}
	}
...
func evalFieldFilter(node *Node, t Row) bool {
	...
	case "tag":
		...
		for _, tg := range t.TagNames {
			if strings.EqualFold(strings.TrimSpace(tg), want) {
				return true
			}
		}
		return false
...
func evalAmountField(node *Node, amt float64) bool {
	switch node.op {
	case "..":
...
func evalDateField(node *Node, dateISO string) bool {
...
func filterExprString(node *Node) string {
	return filterExprStringWithMode(node, false)
}

func filterExprStringWithMode(node *Node, strictGroups bool) string {
	if node == nil {
		return ""
	}
...
func renderFilterNode(node *Node, parentPrec int, strictGroups bool) string {
	if node == nil {
		return ""
	}
...
func renderFieldNode(node *Node) string {
	field := strings.ToLower(node.field)
	switch node.op {
	case "..":
...
func filterNodePrecedence(kind nodeKind) int {
	switch kind {
	case filterNodeOr:
		return 1
	case filterNodeAnd:
...
func needsFilterParens(child *Node, parentPrec int, strictGroups bool, parentKind nodeKind) bool {
	if child == nil {
		return false
	}
...
func flattenFilterChildren(kind nodeKind, in []*Node) []*Node {
...
func filterContainsFieldPredicate(node *Node) bool {
	if node == nil {
		return false
	}
...
func markTextNodesAsMetadata(node *Node) *Node {
	if node == nil {
		return nil
	}
	out := *node
...
func andFilterNodes(nodes ...*Node) *Node {
	...
	return &Node{kind: filterNodeAnd, children: filtered}
}

func orFilterNodes(nodes ...*Node) *Node {
	filtered := make([]*Node, 0, len(nodes))
	for _, n := range nodes {
		if n == nil {
			continue
	...
	return &Node{kind: filterNodeOr, children: filtered}
}
...
func ParseStrict(input string) (*Node, error) {
	return parseFilterStrict(input)
}

func FallbackPlainText(input string) *Node {
	return fallbackPlainTextFilter(input)
}

func Eval(node *Node, row Row) bool {
	return evalFilter(node, row)
}

func String(node *Node) string {
	return filterExprString(node)
}

func StringStrict(node *Node) string {
	return filterExprStringWithMode(node, true)
}

func ContainsFieldPredicate(node *Node) bool {
	return filterContainsFieldPredicate(node)
}

func MarkTextMetadata(node *Node) *Node {
	return markTextNodesAsMetadata(node)
}

func And(nodes ...*Node) *Node {
	return andFilterNodes(nodes...)
}

func Or(nodes ...*Node) *Node {
	return orFilterNodes(nodes...)
}
