...
| Settings | `c` Categories, `t` Tags, `r` Rules, `d` Database, `w` Dashboard Views | Stay (no default reset) |
...
- [x] Jump target selection focuses the correct section; ESC cancels and restores previous focus
...
| `filter:load` | Load Saved Filter | filter_input, transactions |

### Deferred follow-up note (Phase 2 gate)

Saved-filter management UX beyond "save current" / "load most recent" (naming,
review, update, delete, list/apply workflows; CURDL/CURL-style management)
requires a dedicated follow-up spec note. This redesign is intentionally
deferred from v0.32 implementation scope, but must be explicitly tracked in
Phase 2 acceptance to explain why Phase 2 remains unaccepted when the core
parser/evaluator criteria are otherwise met.

### Deferred hardening note: keybinding auto-migration shim (Phase 2 follow-up)

Current v0.32 behavior includes a keybinding materialization shim that auto-rewrites
`manager.quick_tag` if it conflicts with other manager-scope bindings. This was a
stability shortcut carried from pre-v0.4 keybinding hardening and is not a desired
long-term behavior for v0.4 acceptance quality.

Planned rectification (Phase 2 hardening follow-up patch, v0.32.x or earliest v0.33.x):
- Remove silent key mutation in materialization.
- Replace with explicit conflict validation failure and actionable startup guidance.
- Preserve user intent: no behind-the-scenes key reassignment.
...
- [ ] Filter remains responsive while typing (`/` parse + apply <100ms p95 on 10k transactions in harnessed tests).  
  Note: performance harness evidence is not yet captured as a signed-off metric in this section.
- [ ] Saved-filter CURDL/CURL redesign has a dedicated follow-up UX spec section (naming/loading/review/update/delete/list/apply); tracking note exists, but full spec remains a Phase 2 sign-off gate.
- [x] Keybinding materialization no longer performs silent auto-reassignment for manager quick-tag conflicts; conflicts fail explicitly with actionable guidance (Phase 2 hardening follow-up).
...
#### Acceptance status snapshot (v0.32.1)

Snapshot recorded on `v0.32.1` to establish a regression reference point before
follow-up bugfixing.

- [x] `/` opens filter input
- [ ] Plain-text typing behavior is fully preserved (regressions observed with
      global bindings leaking into filter input, e.g. `:`/`v`/tab shortcuts)
- [ ] Field-predicate input/editing flow is fully usable from filter input
      (typing regressions observed)
- [x] Parser/evaluator core behavior (grouping, implicit AND, boolean ops,
      strict/permissive split, strict grouping validation) implemented
- [x] Active filter pill rendering implemented
- [x] Saved filters persist in config and appear as commands in palette
- [ ] In-filter save/load post-apply workflow fully functional (`ctrl+s`/`ctrl+l`
      after Enter-apply requires bugfix pass)
- [x] Invalid saved filters at startup are skipped with actionable warnings
- [x] `f` shortcut removed; filter path is `/`
- [x] Manager account scope composition remains integrated with transaction
      filtering
- [x] Account action modal (`del` => clear txns / nuke account) implemented
- [x] Settings nuke-account command path removed/deprecated
- [x] Account chips show txn count / `Empty`
- [x] DB bootstrap recreates missing DB path/schema
- [x] Saved-filter CURDL/CURL redesign tracking note added; dedicated UX spec
      still pending (intentional acceptance gate)

#### Acceptance status snapshot (v0.32.2)

Snapshot recorded on `v0.32.2` after regression hardening + behavior-validation
review of the test architecture.

- [x] `/` opens filter input and remains active after Enter-apply
- [x] Plain-text typing behavior preserved in filter input (including `:`) with
      no global shortcut leakage while active (`v`, `1-3` no longer hijack input)
- [x] Field-predicate input/editing flow usable from filter input
- [x] Parser/evaluator core behavior implemented (grouping, implicit AND,
      boolean ops, strict/permissive split, strict grouping validation)
- [x] Active filter pill rendering implemented
- [x] Saved filters persist in config and appear as commands in palette
- [x] In-filter save/load post-apply workflow functional (`ctrl+s`/`ctrl+l`
      after Enter-apply)
- [x] Invalid saved filters at startup are skipped with actionable warnings
- [x] `f` shortcut removed; filter path is `/`
- [x] Manager account scope composition remains integrated with transaction
...
- [x] Account chips show txn count / `Empty`
- [x] DB bootstrap recreates missing DB path/schema
- [x] Behavior-validating regression tests added for filter input text safety
      and post-apply save workflow
- [ ] Saved-filter CURDL/CURL dedicated follow-up UX spec section is still
      pending (remaining Phase 2 sign-off blocker)

#### Acceptance status snapshot (v0.32.2b)

Snapshot recorded on `v0.32.2b` (February 13, 2026) after aggressive cleanup
and filter interaction regression hardening.

- [x] Enter-apply in filter input returns control to transaction table navigation
- [x] Post-apply save/load remains available in normal transaction interaction
      flow (`ctrl+s`/`ctrl+l`) without forcing filter edit mode to stay open
- [x] Main acceptance checklist has been reconciled with snapshot-era criteria so
      phase status is tracked in one place
- [ ] Saved-filter CURDL/CURL dedicated follow-up UX section still pending
- [x] Keybinding auto-reassignment shim for manager quick-tag conflict replaced
      by explicit fail-fast validation guidance
