...
> **Keybinding note:** `scopeImportPreview` is a modal scope (see `architecture.md` §3.4.2). It fully blocks all non-modal key handling. Keys `a`/`s`/`c`/`f`/`r`/`p` are safe to bind here without conflicting with any tab or global binding. Add an `overlayEntry` for `importPreviewOpen` in `overlayPrecedence()` (`dispatch.go`) at the same precedence level as the current `importDupeModal` entry — see `architecture.md` §3.2. This modal has no text-editable fields, so no `modalTextContracts` entry is needed. It is a `ContextWorkflow` kind with explicit branch intents (`import:all`, `import:skip-dupes`, `import:cancel`) — footer hints should declare these intents to facilitate Phase 7 contract-driven footer rendering.
