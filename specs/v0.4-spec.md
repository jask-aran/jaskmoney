...
| `filter:clear` | Clear All Filters | transactions |
| `filter:save` | Save Current Filter | filter_input |
| `filter:load` | Load Saved Filter | filter_input |
...
- [ ] In `filter_input` scope, `filter:load` is available when saved filters exist; `filter:save` is available only after a successful Enter-apply of a strict-valid expression
...
- [ ] Regression guards are behavior-validating (not weak-signal): `Update(...)`-driven tests prove filter input consumes printable/global keys as text (`:`, `v`, `1-4`) while active
- [ ] Regression guards prove in-scope post-apply workflow: after Enter-apply in `filter_input`, `ctrl+s`/`ctrl+l` remain functional without mode-switch fallbacks
...
- [x] Settings nuke-account command path removed/deprecated (legacy keybinding
