...
- [Phase 6: Dashboard v2 (v0.36 → v0.4)](#phase-6-dashboard-v2-v036--v04)
...
| `txn:search` | Search Transactions | transactions |
...
**Operator precedence:** `NOT` > `AND` > `OR` (standard). No parentheses in v0.4 to keep it simple.
...
Parse policy is context-sensitive:
- Interactive `/` filter input is **permissive**: parse errors fall back to treating the entire input as plain text search (backward compatible, fast typing).
...
func parseFilterStrict(input string) (*filterNode, error)   // error on any invalid token/shape
```
...
Custom pane modes are stored in `[[dashboard_view]]` blocks in `formats.toml` and appear alongside built-in modes in the `[`/`]` cycle. They are validated with `parseFilterStrict` at load time; invalid expressions are rejected with actionable errors and the mode is not registered.
...
- `filter_test.go` — parser: plain text, field predicates, AND/OR/NOT, ranges, quoted strings, error fallback to text search. Evaluator: all field types, boolean combinations, edge cases (nil category, nil account, empty tags).
...
- `filter_test.go` — strict parser never falls back to text for invalid expressions
...
- Scrollable duplicate list (capped at 100 rows with "+N more" if over 100)
...
- Full view shows all rows with "New" / "DUPE" status column
...
- Scrollable, capped at 100 visible rows with "+N more" indicator
...
5. Enter offset amount (default: full credit amount, capped at remaining unlinked credit)
...
## Phase 6: Dashboard v2 (v0.36 → v0.4)
...
3. Save current dashboard state to `drillReturn`: tab, focused widget, active mode, scroll
...
Phase 1: Command System Overhaul + Jump Mode (v0.31)
    ...
    │               │
    │               └── Phase 6: Dashboard v2 (v0.36 → v0.4)  [needs Phase 1 jump mode + Phase 2 + Phase 5]
    │
    └── (Phase 2 also needs Phase 1 for command-based filter actions)
```

**Critical path:** Phase 1 → Phase 2 → Phase 3 → Phase 4 (serial).
**Parallel after Phase 2:** Phase 3 and Phase 5 can proceed in parallel once Phase 2 lands.
...
**Migration timing:** schema v5 migration ships with the first release that includes Phase 3 or Phase 5 DB changes (preferred: v0.33). Budget tables may exist before Budget UI is enabled.
...
2. `go vet ./...` clean
3. `go test ./...` all pass
4. `go test -tags flowheavy ./...` passes
5. Performance checks pass for interactive flows (especially filter typing/render responsiveness target from Phase 2)
6. Manual smoke test of the new feature
7. Tag the sub-release (e.g. `git tag v0.31`)

Final v0.4:
- Full end-to-end test: create filter expression → save filter → create rule using filter → dry-run → apply rules → import with preview → set budget → view budget table + analytics strip → planner view → configure custom pane mode in Settings → dashboard focus via jump mode → mode cycling through curated + custom → drill-down to Manager → ESC returns to dashboard with restored state → verify filter cleared
