...
══════════════════════════════════
  ...
  #   Date         Amount    Description            Status   Category   Tags (post-rules)
  1   2025-01-03   -$42.50   COUNTDOWN METRO        New      Groceries  essentials
  2   2025-01-03   +$500.00  SALARY DEPOSIT         New      Income     salary,payroll
  3   2025-01-05   -$42.50   COUNTDOWN METRO        DUPE     Groceries  essentials
  4   2025-01-05   -$12.30   UBER *EATS             DUPE     Dining     takeaway
  ...

  [r] Raw View   [p] Post-Rules View   [a] Import All   [s] Skip Dupes   [Esc] Back
```

- Full view shows all previewed rows in stable file order with `New` / `DUPE` status.
- `r` toggles raw values; `p` toggles post-rules projection.
- Post-rules mode renders both projected category and projected tags.
- Full view is scrollable, display-capped to first 100 rows with `+N more not shown`.
- Decision keys remain available in full view (`a`/`s`) to avoid unnecessary context switching.
- `Esc` in full view returns to compact view (does not cancel immediately).

**Esc lifecycle contract:**
- `Esc` in full view -> compact view
- `Esc` in compact view -> cancel import and close modal
...
- Confirm import uses the same frozen snapshot and same locked ruleset to guarantee preview/import parity.
...
- `Esc` step-down lifecycle governs navigation/cancel
...
- `render.go`
  - replace `renderDupeModal` with `renderImportPreview` (compact + full + raw/post-rules)
  - render category + tags in post-rules full view
  - render parse error summary and decision-disabled state
- `keys.go`
  - add `scopeImportPreview`
  - bind import preview actions (`a`, `s`, `f`, `r`, `p`, `esc`) and standard up/down navigation aliases
...
- `model_test.go`
  - compact/full toggle, raw/post-rules toggle, scroll behavior, decision execution
- `render_test.go`
  - compact/full rendering capped at 100 rows with `+N more` indicator
  - dupe/new status rendering, category+tags rendering in post-rules mode
...
- [ ] Import flow opens combined preview modal for every scan result (including zero-duplicate files)
- [ ] Compact view shows summary + scrollable duplicate list in stable file order
- [ ] Full view shows all displayed rows with New/DUPE status; post-rules mode shows projected category **and tags**
- [ ] `a`/`s` decisions work in compact and full view; `Esc` lifecycle is full->compact->cancel
- [ ] `c` is not bound as a cancel shortcut in import preview
- [ ] Navigation uses standard up/down action surface (default aliases include vim/arrows/ctrl+p/n)
- [ ] Preview/import parity is guaranteed by frozen snapshot import path (no reparse-on-confirm)
- [ ] Preview/import parity is guaranteed by locked ruleset captured at preview-open
- [ ] Post-rules preview runs on all rows asynchronously without DB writes
- [ ] Any parse/normalization errors block import decisions and are shown with row-level diagnostics
- [ ] Row display capped at 100 with `+N more` indicator in compact and full views
- [ ] While preview is open, rule edits/actions are blocked by modal precedence
- [ ] Command-open gating derives from overlay state and blocks command UI while preview modal is active
- [ ] Update-driven flow tests and failure-path no-partial-write regressions are added per architecture §8
- [ ] `specs/architecture.md` and `AGENTS.md` are updated in the same commit to reflect finalized import-preview behavior
