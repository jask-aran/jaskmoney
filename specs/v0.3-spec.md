...
**New model fields:**
- `dashTimeframe int` (thisMonth/lastMonth/3M/6M/YTD/1Y/custom)
- `dashTimeframeFocus bool`
- `dashCustomStart`, `dashCustomEnd`, `dashCustomInput string`

**Render:** Row of chips at top of dashboard: `[This Month] [Last Month] [3M] [6M] [YTD] [1Y] [Custom]`. Active chip highlighted with `colorAccent`. Focused chip has cursor.
...
- `Shift+Space` — select range from last-selected to cursor
- `Esc` (when selections exist) — clear all selections

**Render:** Selected rows get a `*` prefix. Cursor+selected gets `*>`. Selection count shown in title: `Transactions (3 selected)`.

**Key detail:** Selection is by transaction ID, not index, so it survives re-filtering/re-sorting.

**Tests:** Toggle, range select, clear, persistence across filter changes.

**Acceptance:**
- Space toggles, Shift+Space ranges, Esc clears
- Visual indicator on selected rows
